      ...
      if (empty) {
        ...
        if (hint) {
          ...
          hint.textContent = bits.length
            ? "Aucun bien pour " + bits.join(", ") + ". Envoyez un lien d'annonce ou confiez-moi la recherche locale."
            : "Envoyez un lien d'annonce : je regarde si le bien entre dans mon secteur de recherche.";
        }
      }
      return;
    }
