#!/usr/bin/env node
/**
 * Statut du plan d'articles evergreen orientés leads.
 *
...
function main() {
  ...
  console.log("Plan mis à jour:", plan.updated || "n/a");
  ...
  console.log("Sujets planifiés:", topics.length);
  console.log("Déjà publiés (state):", done.size);
  console.log("Restants publiables:", remaining.length);
  console.log("Runs enregistrés:", (state.runs || []).length);

  if (next.length) {
    console.log("\nProchains sujets:");
    next.forEach(function (topic, i) {
      console.log(
        "  " + (i + 1) + ". [" + (topic.priority || 0) + "] " + topic.id + " — " + topic.title
      );
    });
  } else {
