#!/usr/bin/env node
/**
 * Pipeline evergreen leads : plan editorial -> article questionnaire -> publish.
 *
 * Usage:
...
function relatedLinks(topic) {
  ...
  var extras = {
    ...
    finance: [{ href: "../assurance-emprunteur/", label: "Assurance emprunteur" }],
    sante: [{ href: "../assurance-sante/", label: "Mutuelle sante" }],
    ...
    blocks: [
      {
        type: "p",
        text:
          "Ce guide vise une situation concrete : <strong>" +
          topic.intent +
          "</strong>. L'objectif n'est pas de lire un comparatif de plus, mais de savoir si votre contrat actuel colle encore a votre risque, a votre budget et aux exigences du marche.",
      },
      {
        type: "h2",
        text: "Pourquoi ce sujet genere des demandes qualifiees",
      },
      {
        type: "p",
        text:
          topic.pain +
          " C'est exactement le moment ou un questionnaire court transforme une lecture blog en demande exploitable : besoin, profil, niveau de garantie et urgence sont clarifies avant l'appel.",
      },
      {
        type: "h2",
      ...
      },
      {
        ...
        text:
          ...
          trigger +
          "</strong>. Si l'un de ces cas vous concerne, comparer maintenant evite de choisir dans l'urgence apres un sinistre, un refus d'attestation ou une facture trop elevee.",
      },
      {
        type: "h2",
        text: "Les garanties a verifier avant de signer",
      },
      {
        type: "ul",
        items: checklist,
      },
      {
        type: "p",
        text:
          "Ne comparez pas uniquement le prix. Deux contrats au meme tarif peuvent etre tres differents si les franchises, exclusions, plafonds ou delais de carence ne correspondent pas a votre profil.",
      },
      { type: "bridge" },
      {
        type: "h2",
      ...
      },
      {
        type: "p",
        text:
          "Le questionnaire Leads Opportunities sert a qualifier le besoin en 3 a 6 minutes : situation, budget, garanties indispensables et date d'effet souhaitee. Il permet ensuite d'orienter vers le bon parcours, sans multiplier les formulaires inutiles.",
      },
      {
        type: "h2",
      ...
      },
      {
        ...
        text:
          ...
          topic.leadPromise +
          "</strong>. Un conseiller peut ensuite comparer plusieurs assureurs ou courtiers partenaires selon vos reponses.",
      },
    ],
    faq: topic.faq || [],
...
async function main() {
  ...
  var count = Math.min(maxCount, Math.max(1, Number(arg("count", plan.cadence && plan.cadence.defaultCount || 1)) || 1));
  ...
  console.log("✓ Articles lead generes:", generated.length);
}
