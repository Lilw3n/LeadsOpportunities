...
function validateArticle(article) {
  ...
  if (isPlaceholderQueueItem(article)) {
    errors.push("titre gabarit / placeholder (COLLEZ ICI)");
  }
  return errors;
}
...
function readStdinIfPiped() {
  ...
  try {
    ...
    return stdin && String(stdin).trim() ? String(stdin) : "";
  } catch (e) {
    return "";
  }
}
