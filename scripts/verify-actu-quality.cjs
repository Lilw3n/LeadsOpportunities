...
function validateArticle(article) {
  ...
  else if (/collez ici/i.test(String(article.title))) {
    errors.push("titre placeholder (file Cafeyn) — ne pas publier");
  }
...
function main() {
  ...
  if (file) {
    ...
    if (!stdin) {
      console.error("stdin vide — passez un JSON d'article(s).");
      process.exit(1);
    }
