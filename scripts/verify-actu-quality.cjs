...
function validateArticle(article) {
  ...
  if (isPlaceholderActuItem(article)) errors.push("titre placeholder (file inbox)");
...
function loadArticlesFromStdinOrPending() {
  var stdin = "";
  ...
  if (stdin.trim()) {
    ...
    return Array.isArray(parsed) ? parsed : parsed.articles || [parsed];
  }
...
function main() {
  ...
  if (file) {
    ...
    articles = loadArticlesFromStdinOrPending();
  }
