...
function validateArticle(article) {
  ...
  else if (/collez ici|\[titre|placeholder|votre titre ici/i.test(article.title)) {
...
function main() {
  ...
  if (file) {
    ...
    if (String(stdin).trim()) {
      ...
      console.error("stdin vide");
      process.exit(1);
    } else {
      // CI / execSync : stdin n'est pas un TTY mais est vide → lire pending
