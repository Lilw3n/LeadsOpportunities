...
function validateArticle(article) {
  ...
  else if (/^collez ici\b/i.test(String(article.title)) || /\b(TODO|FIXME|placeholder)\b/i.test(String(article.title))) {
    errors.push("titre placeholder (file manuelle d'exemple)");
  }
...
function main() {
  ...
  if (file) {
    ...
    if (!stdin) {
      console.error("stdin vide — passez un JSON article ou utilisez --file=");
      process.exit(1);
    }
