...
function readArticlesFromStdin(force) {
  if (!force && process.stdin.isTTY) return null;
  try {
...
function main() {
  ...
  if (file) {
    ...
    articles = readArticlesFromStdin(forceStdin);
    if (forceStdin && !articles) {
      console.error("Aucun JSON reçu sur stdin.");
      process.exit(1);
    }
    articles = articles || loadPendingArticles();
  }
