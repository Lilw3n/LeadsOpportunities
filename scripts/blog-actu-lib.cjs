...
const { PRIORITY_SOURCE_TYPES, resolveSourceType } = require("./blog-actu-sources.cjs");
...
function scoreLeadPotential(candidate) {
  ...
  if (PRIORITY_SOURCE_TYPES.indexOf(resolveSourceType(candidate.sourceType || candidate.source)) !== -1) {
    score += 12;
  }
