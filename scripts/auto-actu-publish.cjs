...
const { readJson, writeJson, rankCandidates, appendPendingArticle, isPlaceholderQueueItem, isLowLeadActuTopic, hasLeadConversionAngle } = require("./blog-actu-lib.cjs");
...
function pickCandidates(candidates, count, state) {
  ...
  var available = ranked.filter(function (c) {
    ...
    if (isLowLeadActuTopic(c)) return false;
