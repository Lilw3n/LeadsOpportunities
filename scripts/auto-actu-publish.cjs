...
function candidateSourceType(c, feedMap) {
  ...
  return normalizeSourceType(c.source || c.feedName || c.feedId, feedMap[c.feedId] || "aggregator");
}
...
function pickCandidates(candidates, count, state) {
  ...
  if (count >= 3) {
    var coverageCount = Math.min(count, PLATFORM_TYPES.length);
    PLATFORM_TYPES.slice(0, coverageCount).forEach(function (platform) {
      if (picks.length >= count) return;
    ...
    state._nextPlatformRotation = ((state.platformRotationIndex || 0) + coverageCount) % PLATFORM_TYPES.length;
  } else {
