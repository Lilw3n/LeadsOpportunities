...
const { readJson, writeJson, rankCandidates, appendPendingArticle, isPlaceholderQueueItem } = require("./blog-actu-lib.cjs");
...
async function main() {
  ...
  for (var i = 0; i < picks.length; i++) {
    var pick = picks[i];
    if (isPlaceholderQueueItem(pick)) {
      console.warn("  Placeholder inbox — ignoré");
      continue;
    }
