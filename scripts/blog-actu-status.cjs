 ...
 */
const { readJson, loadPendingArticles, rankCandidates, isAutopublishLeadCandidate } = require("./blog-actu-lib.cjs");
...
function main() {
  ...
  if ((candidates.candidates || []).length) {
    var rankedLeads = rankCandidates(candidates.candidates || []).filter(isAutopublishLeadCandidate);
    console.log("\n--- Top candidats leads ---");
    (rankedLeads.length ? rankedLeads : candidates.candidates).slice(0, 5).forEach(function (c, i) {
