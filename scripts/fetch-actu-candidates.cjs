#!/usr/bin/env node
/**
 * Récupère l'actu (RSS publics) + file d'attente manuelle multi-source.
...
function mergeWithQuotas(buckets, quotas) {
  var merged = [];
  sourceTypesForBuckets(quotas).forEach(function (type) {
...
function ingestQueueItem(item, buckets, processed) {
  ...
  var queueType = resolveSourceTypeFromText(item.source);
...
async function processFeed(feed, buckets, processed, maxPerFeed) {
  var sourceType = normalizeSourceType(feed.sourceType || feed.name || feed.id);
...
async function main() {
  var feedsCfg = readJson("blog-actu-feeds.json", { feeds: [], quotas: DEFAULT_SOURCE_QUOTAS });
  var quotas = Object.assign({}, DEFAULT_SOURCE_QUOTAS, feedsCfg.quotas || {});
  ...
  var buckets = createBuckets(quotas);
  ...
  writeJson("blog-actu-candidates.json", {
    ...
    quotas: quotas,
    bySource: sourceTypesForBuckets(quotas).reduce(function (acc, type) {
