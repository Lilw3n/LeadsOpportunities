...
const SOURCE_ORDER = ["cafeyn", "edge", "firefox", "google", "yahoo", "aggregator"];
...
function orderedSourceTypes(quotas, buckets) {
  var seen = new Set();
  var ordered = [];
  ...
  Object.keys(quotas || {}).concat(Object.keys(buckets || {})).forEach(function (type) {
    if (!seen.has(type)) {
      seen.add(type);
      ordered.push(type);
    }
...
function createBuckets(feeds, quotas) {
  var buckets = {};
  orderedSourceTypes(quotas, {}).forEach(function (type) {
    buckets[type] = [];
  });
  (feeds || []).forEach(function (feed) {
    buckets[feed.sourceType || "aggregator"] = buckets[feed.sourceType || "aggregator"] || [];
  });
  return buckets;
}
...
function mergeWithQuotas(buckets, quotas) {
  var merged = [];
  orderedSourceTypes(quotas, buckets).forEach(function (type) {
...
async function main() {
  ...
  var buckets = createBuckets(feedsCfg.feeds || [], quotas);
  ...
  writeJson("blog-actu-candidates.json", {
    ...
    quotas: quotas,
    bySource: orderedSourceTypes(quotas, buckets).reduce(function (acc, type) {
  ...
  console.log(
    ...
    deduped.length,
    "—",
    orderedSourceTypes(quotas, buckets)
      .map(function (type) {
