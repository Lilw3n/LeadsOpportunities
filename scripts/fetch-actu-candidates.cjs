...
async function fetchText(url) {
  var res = await fetch(url, {
    headers: {
      "User-Agent": "Mozilla/5.0 (compatible; LeadsOpportunities-BlogBot/1.0; +https://www.leadsopportunities.fr)",
...
async function processFeed(feed, buckets, processed, maxPerFeed) {
  ...
  try {
    var attempts = String(feed.url || "").indexOf("bing.com/news") !== -1 ? 3 : 1;
    var items = [];
    for (var attempt = 0; attempt < attempts; attempt++) {
      var xml = await fetchText(feed.url);
      items = parseRssItems(xml).slice(0, maxPerFeed);
