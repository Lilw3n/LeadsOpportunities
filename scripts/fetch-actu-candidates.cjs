...
async function processFeed(feed, buckets, processed, maxPerFeed) {
  var sourceHint = feed.sourceType === "aggregator" ? feed.name || feed.id : feed.sourceType || feed.name || feed.id;
  var sourceType = normalizeSourceType(sourceHint);
...
async function main() {
  ...
  writeJson("blog-actu-candidates.json", {
    ...
    quotas: quotas,
    bySource: bySourceCounts(buckets),
    candidates: deduped,
  });
  ...
  console.log("Candidates:", deduped.length, "—", formatSourceCounts(buckets));
  if (deduped.length) {
    console.log("Top 3:");
