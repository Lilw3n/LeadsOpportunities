...
async function processFeed(feed, buckets, processed, maxPerFeed) {
  var sourceType = feedSourceType(feed);
...
async function main() {
  ...
  writeJson("blog-actu-candidates.json", {
    ...
    quotas: quotas,
    bySource: sourceCountsFromBuckets(buckets),
    candidates: deduped,
  });
  ...
  console.log("Candidates:", deduped.length, "—", formatSourceCounts(sourceCountsFromBuckets(buckets)));
  if (deduped.length) {
    console.log("Top 3:");
