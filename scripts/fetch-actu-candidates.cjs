...
function mergeWithQuotas(buckets, quotas) {
  var merged = [];
  activeSourceTypes(quotas).forEach(function (type) {
...
function normalizedCandidateKey(candidate) {
  var file = String(candidate.suggestedFile || "").toLowerCase();
...
function dedupeMergedCandidates(candidates) {
  var seen = new Set();
  return candidates.filter(function (candidate) {
    var key = normalizedCandidateKey(candidate);
...
async function processFeed(feed, buckets, processed, maxPerFeed) {
  var sourceType = normalizeSourceType([feed.sourceType, feed.name, feed.id].join(" "));
...
async function main() {
  ...
  var deduped = dedupeMergedCandidates(mergeWithQuotas(buckets, quotas));
  var bySource = {};
  SOURCE_TYPES.forEach(function (type) {
