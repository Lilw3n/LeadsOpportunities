...
function ingestQueueItem(item, buckets, processed) {
  ...
  if (!scaffold) return;
  queueType = ensureBucket(buckets, queueType);
...
async function processFeed(feed, buckets, processed, maxPerFeed) {
  var sourceType = ensureBucket(buckets, resolveSourceType(feed.sourceType || feed.name, feed.id));
  try {
...
async function main() {
  ...
  writeJson("blog-actu-candidates.json", {
    ...
    quotas: quotas,
    bySource: countsBySource(buckets),
    candidates: deduped,
  });
  ...
  console.log(
    "Candidates:",
    deduped.length,
    "— cafeyn/edge/firefox/google/bing/yahoo/aggr:",
