...
function candidateKey(candidate) {
  var raw = String(candidate.url || "").trim();
  if (!raw) return candidateTitleKey(candidate);
  try {
    var parsed = new URL(raw);
    if (parsed.hostname.indexOf("bing.com") !== -1 && parsed.pathname.indexOf("/news/apiclick") !== -1) {
      var target = parsed.searchParams.get("url");
      if (target) return candidateKey({ url: target, title: candidate.title });
    }
    ["utm_source", "utm_medium", "utm_campaign", "utm_content", "utm_term", "oc"].forEach(function (param) {
      parsed.searchParams.delete(param);
    });
    parsed.hash = "";
    return parsed.toString().toLowerCase().replace(/^https?:\/\/(www\.)?/, "").trim();
  } catch (e) {
    return raw
      .toLowerCase()
      .replace(/\s+/g, " ")
...
function candidateTitleKey(candidate) {
...
function mergeWithQuotas(buckets, quotas) {
  ...
  sourceTypes().forEach(function (type) {
    ...
    list.forEach(function (candidate) {
      ...
      var keys = [candidateKey(candidate), candidateTitleKey(candidate)].filter(Boolean);
      if (keys.some(function (k) { return seen.has(k); })) return;
      keys.forEach(function (k) { seen.add(k); });
      picked.push(candidate);
    });
    merged = merged.concat(picked);
  });
  return merged;
}
