# Mise en route — actu auto + Cafeyn (15 min)
...
## Étape 4 — environnement d'exécution (3 min)

Configurez les mêmes variables dans l'outil qui lancera `npm run blog:actu:auto` (Cursor Automation, cron serveur, ou GitHub Actions si vous ajoutez un workflow dédié).
...
Cette branche n'ajoute pas de workflow GitHub imposé : planifiez simplement la commande npm dans l'outil choisi.
...
4. Au prochain run planifié, article **prioritaire** proposé/publié sur le blog

---

## Étape 6 — Vérifier que ça tourne

Lancez la commande dans Cursor, en local, ou dans votre outil de planification :
...
| « DATABASE_URL requis » | Variable Vercel + variable de l'environnement d'exécution + SQL Neon exécuté |
| Pas d’article Cafeyn spécifique | Utiliser le favori ; le RSS ne voit que les flux publics |
| Qualité insuffisante | Ajouter `GEMINI_API_KEY` dans l'environnement qui lance le pipeline |
