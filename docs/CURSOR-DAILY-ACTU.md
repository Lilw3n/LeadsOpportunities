...
2. npm run blog:actu:auto -- --count=1
   (ou --count=5 pour couvrir Cafeyn, Edge, Firefox, Google News et Bing en une fois)
...
Règles :
- Ne jamais demander ni stocker de login Cafeyn ; utiliser RSS publics + inbox sécurisée
...
| `--count` | Comportement |
|-----------|--------------|
| `1` | 1 article par run ; rotation **Cafeyn → Edge → Firefox → Google → Bing** sur la journée |
| `3` | **1 Cafeyn + 1 Edge + 1 Firefox** si disponibles |
| `5` | Ajoute aussi **Google News + Bing News** si disponibles ; Yahoo reste en complément scoré |

Exemple **5×/jour multi-sources** : 5 automations à `count=1` (rotation auto) **ou** 1–2 runs/jour à `count=3` / `count=5`.
