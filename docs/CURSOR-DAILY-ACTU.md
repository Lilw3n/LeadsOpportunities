...
Objectif : publier des articles actu Cafeyn + Edge + Firefox + Google/Bing/Yahoo vers les questionnaires.

Étapes obligatoires :
1. npm install
2. npm run blog:actu:auto -- --count=1
   (ou --count=3 pour les 3 plateformes principales ; --count=5 pour ajouter Google/Bing/Yahoo par rotation)
...
| `4` ou `5` | Les 3 plateformes principales + Google/Bing/Yahoo sur les créneaux restants |

Exemple **5×/jour multi-sources** : 5 automations à `count=1` (rotation auto) **ou** 1–2 runs/jour à `count=3/5`.
