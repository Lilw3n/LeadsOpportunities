...
Objectif : publier des articles actu Cafeyn + Edge + Firefox + Google News + Bing (Yahoo en complément) vers les questionnaires.
...
| `--count` | Comportement |
|-----------|--------------|
| `1` | 1 article par run ; rotation **Cafeyn → Edge → Firefox → Google News → Bing** sur la journée (5 runs = 5 articles) |
| `3` | **1 Cafeyn + 1 Edge + 1 Firefox** si disponibles |
| `5` | Ajoute aussi **Google News + Bing** si disponibles |

Exemple **5×/jour multi-sources** : 5 automations à `count=1` (rotation auto) **ou** 1 run/jour à `count=5`.
