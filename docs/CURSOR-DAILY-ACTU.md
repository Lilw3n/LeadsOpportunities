...
2. npm run blog:actu:auto -- --count=1
   (ou --count=5 pour couvrir Cafeyn, Edge, Firefox, puis Google/Bing/Yahoo en rotation)
...
| `--count` | Comportement |
|-----------|--------------|
| `1` | 1 article par run ; rotation sur les plateformes disponibles |
| `3` | **1 Cafeyn + 1 Edge + 1 Firefox** à chaque run si le vivier le permet |
| `5` | Ajoute Google News / Bing News / Yahoo en rotation pour élargir la veille |

Exemple **5×/jour** : 5 automations à `count=1` (rotation auto) **ou** 1–2 runs/jour à `count=5` pour couvrir davantage de sources.
