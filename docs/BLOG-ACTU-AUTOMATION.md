...
# Les 3 plateformes principales en une fois
npm run blog:actu:auto -- --count=3

# Test large : Cafeyn + Edge + Firefox + rotation Google/Bing/Yahoo
npm run blog:actu:auto -- --dry-run --count=5 --no-ai
...
Configuration des flux : **`data/blog-actu-feeds.json`** (78 sources testées).
