...
Chaque exécution reprend explicitement les grandes sources d'actualité utiles au blog :
...
| **Edge** | Page d'accueil Edge / MSN représentée par Bing News : France, actu, économie, assurance, mutuelle, immobilier, santé |
| **Firefox** | Page d'accueil Firefox représentée par France Info, 20 Minutes, France 24, Mediapart, RFI, BFMTV, Europe 1, HuffPost, Courrier international + Pocket |
| **Google News** | 20+ requêtes assurance + **Coupe du monde 2026** (matchs, Bleus, Mbappé, supporters, voyage) |
| **Bing News** | Une France, assurance, mutuelle, crédit conso, sinistres, immobilier / emprunteur |
| **Yahoo** | Yahoo Actualités France + Yahoo Finance France |

**Sélection** :
- `--count=3` → **1 article Cafeyn + 1 Edge + 1 Firefox** à chaque run
- `--count=5` → ajoute Google News et Bing si des candidats qualifiés existent
- `--count=1` → rotation automatique (cafeyn → edge → firefox → google → bing → yahoo)

Les candidats Google/Bing/Yahoo ont leurs propres quotas et restent filtrés par score de lead France.
...
| **Edge** (MSN actu) | Requêtes Bing News RSS (`sourceType: edge`) |
| **Firefox** (France Info, 20 Minutes, Pocket) | RSS Franceinfo + 20 Minutes + API Pocket optionnelle (`sourceType: firefox`) |
| **Google News** | RSS Google News France et requêtes assurance (`sourceType: google`) |
| **Bing** | RSS Bing News thématiques (`sourceType: bing`) |
...
Configuration des flux : **`data/blog-actu-feeds.json`** (~65 sources testées).
