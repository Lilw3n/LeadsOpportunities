...
Objectif : publier **1 à 5 articles par jour** liés à l’actualité (équivalent Cafeyn, Edge/Bing, Firefox/Pocket, Google News, Yahoo) avec CTA **questionnaires** et leads qualifiés — **sans login Cafeyn**.
...
Chaque exécution **reprend explicitement Cafeyn, Edge/Bing, Firefox/Pocket, Google News et Yahoo** :
...
| **Edge** | Bing News : France, actu, économie, assurance, mutuelle, immobilier, santé, sport |
| **Firefox** | France Info (titres/santé/éco), France 24, Mediapart, RFI, BFMTV, Europe 1, HuffPost, 20 Minutes, Courrier international + Pocket |
| **Google News** | 20+ requêtes assurance + actualité France + **Coupe du monde 2026** (matchs, Bleus, Mbappé, supporters, voyage) |
| **Yahoo** | Yahoo Actualités France + Yahoo Finance France |

**Sélection** :
- `--count=5` → vise **1 article par canal** : Cafeyn + Edge/Bing + Firefox/Pocket + Google News + Yahoo
- `--count=3` → 3 canaux selon la rotation courante, puis complément par score lead
- `--count=1` → rotation automatique (cafeyn → edge → firefox → google → yahoo) sur les 5 crons/jour

Les autres agrégateurs restent en secours et ne remplacent pas les canaux principaux quand un candidat exploitable existe.

```bash
# 1 article (rotation cafeyn/edge/firefox/google/yahoo)
npm run blog:actu:auto

# Les 5 canaux en une fois (recommandé pour test large)
npm run blog:actu:auto -- --count=5

# Trois articles selon la rotation
...
| **Edge / Bing** (page d'accueil Edge, Bing Actu) | Flux RSS publics Bing News (`sourceType: edge`) |
| **Firefox / Mozilla** (Pocket, France Info, 20 Minutes…) | RSS éditeurs + Pocket optionnel (`sourceType: firefox`) |
| **Google News** | RSS publics Google News (`sourceType: google`, détecté aussi depuis `google-news-*`) |
| **Yahoo** | RSS publics Yahoo Actualités/Finance (`sourceType: yahoo`) |
...
Configuration des flux : **`data/blog-actu-feeds.json`** (~60 sources testées).
...
RSS + queue manuelle + Pocket
        ↓
blog-actu-candidates.json (score leadScore + buckets cafeyn/edge/firefox/google/yahoo)
        ↓
auto-actu-publish (rotation cafeyn → edge → firefox → google → yahoo → secours aggregator)
        ↓
