...
- **RSS** des mêmes journaux (15 flux actifs `sourceType: cafeyn`, hors endpoints publics cassés)
...
Chaque exécution reprend les sources prioritaires **Cafeyn, Edge, Firefox, Google News et Bing**, puis Yahoo / agrégateurs en complément :

| Plateforme | Flux utilisés |
|------------|---------------|
| **Cafeyn** | Figaro, Parisien, Libé, Ouest-France, Midi Libre, La Dépêche, Nice-Matin, DNA, Le Progrès, Le Monde, L'Express… |
| **Edge** | Flux publics équivalents MSN / Bing News : France, actu, économie, assurance, mutuelle, immobilier, santé |
...
| **Bing News** | Requêtes dédiées France, assurance, prêt immobilier, sinistres |
| **Yahoo** | Yahoo Actualités France + Yahoo Finance |

**Sélection** :
- `--count=3` → **1 article Cafeyn + 1 Edge + 1 Firefox** si disponibles
- `--count=5` → ajoute aussi **Google News + Bing News** si disponibles
- `--count=1` → rotation automatique (cafeyn → edge → firefox → google → bing) sur les crons

Les candidats Yahoo restent en complément scoré : ils peuvent être publiés si leur angle France / assurance dépasse les autres sources.

```bash
# 1 article (rotation cafeyn/edge/firefox/google/bing selon l'heure)
...
| **Edge** (MSN actu) | Requêtes Bing News RSS typées `edge` |
| **Firefox** (France Info, 20 Minutes) | RSS Franceinfo + 20 Minutes + Pocket |
| **Google News** | RSS Google News France / requêtes assurance (`google`) |
| **Bing** | RSS Bing News France / requêtes assurance (`bing`) |
| **Yahoo** | RSS Yahoo Actualités et Yahoo Finance (`yahoo`) |
...
Configuration des flux : **`data/blog-actu-feeds.json`** (sources publiques testables sans compte).
...
blog-actu-candidates.json (score leadScore)
        ↓
auto-actu-publish (rotation cafeyn → edge → firefox → google → bing, Yahoo en complément)
        ↓
